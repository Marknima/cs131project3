...
class Interpreter(InterpreterBase):
  ...
  def run(self, program):
    ...
    self.lambda_manager = {}
    # lambda_manager[lambda_func_name][calling_func_name] = snapshot from getAllVariables()
    # calling_func_name = "resultf", "f", "funcname"
      # assign f resultf
      # funccall e
  ...
  def print_vars(self):
    print('MAIN VAR TABLE:')
    main_func = self.env_manager.environment[0][0]
    if len(main_func) > 0:
      for var in main_func:
        print(f'{var}: {main_func[var]}')
        if main_func[var].type() == Type.OBJECT and main_func[var].value() != None:
          members = main_func[var].value()
  ...
  def _process_line(self):
    ...
    args = tokens[1:]
    #print("run through process line")
    match tokens[0]:
...
#I added a little bit to accommodate for FUNC
  def _assign(self, tokens):
    ...
    #print("run through assign")
   #print(value_type)
  # type_of_vname = vname.type()               ##
   #if type_of_vname == Type.FUNC:                ##
  #  if self.is_function(tokens[1:]):               ##
      #assign tokens[1:] to vname
   #   self.env_manager.set(vname, tokens[1:])         #
    
    obj = tokens[0].split('.') # check if object member
    if len(obj) == 1: # if not an object member
      ...
      self._advance_to_next_statement()
    else: # variable is an object member, so call accordingly
      obj_name = obj[0]
      member_name = obj[1]
      ...
      if obj_members == None:
        obj_members = {}
      obj_members[member_name] = value_type
      updated_obj = Value(Type.OBJECT, obj_members)
      self._set_value(obj_name, updated_obj)
      self._advance_to_next_statement()
      

  def _funccall(self, args):
    #print("run through funccall")
    if not args:
    ...
    if args[0] == InterpreterBase.PRINT_DEF:
      ...
      #print(args)
      #print("from line 176")
      funcVal = self._get_value(args[0])
      
      if funcVal.type() is not Type.FUNC:
        super().error(ErrorType.TYPE_ERROR,"funcVal Type error", self.ip)

      calling_func_name = args[0]
      args[0] = funcVal.value()
      if args[0] == "Empty_function":
        return self._advance_to_next_statement()
      self.return_stack.append(self.ip+1)
      
      self._create_new_environment(args[0], args[1:], calling_func_name)  # Create new environment, copy args into new env
      self.ip = self._find_first_instruction(args[0])

  # create a new environment for a function call
  def _create_new_environment(self, funcname, args, calling_func_name):
    ...
    for formal, actual in zip(formal_params.params,args):
      ...
      if formal_typename in self.reference_types or formal_typename == InterpreterBase.OBJECT_DEF:
        tmp_mappings[formal_name] = arg 
      else:
        tmp_mappings[formal_name] = copy.copy(arg)

    obj = calling_func_name.split('.')
    if len(obj) > 1:
      newObj = self._get_value(obj[0])
      tmp_mappings['this'] = newObj
    
    # for passing surrounding environment to lambda function
    all_vars = self.env_manager.getAllVariables() 
    for var in all_vars:
      # copy each var in lambda snapshot to tmp_mappings
      pass
  ...
  def _setup_default_values(self):
    ...
    self.type_to_default[InterpreterBase.FUNC_DEF] = Value(Type.FUNC,"Empty_function") #I don't know the default for Func type, what could it be?
  ...
  def _get_value(self, token):
    ...
    if len(obj) > 1:
      obj_name = obj[0]      
      member_name = obj[1]
      obj = self._get_value(obj_name) # validate that obj_name is an object and shit
      obj_members = obj.value()
      # validate that obj_members is empty or not and shit
      if obj_members == None:
        super().error(ErrorType.NAME_ERROR,f"Empty object", self.ip)
      if member_name in obj_members:
        member_value = obj_members[member_name] # -> Value(type, value)
        return member_value
  ...
  def getType(self, args):
    arg = self._get_value(args[1:])
    return args.type()

# for when we want to access the object's member variables
  def _parse_dot(self, name):
    parts = name.split(".")
    if len(parts) == 1:
      return name, None
    return parts
